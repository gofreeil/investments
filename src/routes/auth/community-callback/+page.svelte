<script lang="ts">
	...
	async function joinWith(provider: 'google' | 'facebook') {
		...
		try {
			await signIn(provider, { callbackUrl: data.returnTo || '/' });
		} catch {
			oauthLoading = null;
		}
	}
	...
	onMount(async () => {
		...
		// ה-SSO שלנו נקרא "gofreeil-sso", לכן פונים ישירות לכתובת ה-callback,
		...
		try {
			...
			const res = await fetch('/auth/callback/gofreeil-sso', {
				...
				body: new URLSearchParams({ callbackUrl: callbackUrl.href })
			});
		...
		{:else}
			...
			<div class="flex flex-col gap-2.5">
				...
				<button
					...
					class="flex w-full items-center justify-center gap-3 rounded-xl bg-white px-4 py-3 font-bold text-gray-900 shadow-lg transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-60"
				>
				...
				<button
					...
					disabled={oauthLoading !== null}
					class="flex w-full items-center justify-center gap-3 rounded-xl bg-[#1877F2] px-4 py-3 font-bold text-white shadow-lg transition hover:bg-[#166FE5] disabled:cursor-not-allowed disabled:opacity-60"
				>
				...
				{/if}
				<a href="/register?redirect={encodeURIComponent(data.returnTo || '/')}" class="mt-1 text-sm text-gray-400 underline hover:text-gray-200">
					מעדיפים אימייל וסיסמה? הרשמה ידנית
				</a>
				<a href="/login?redirect={encodeURIComponent(data.returnTo || '/')}" class="text-sm text-gray-500 underline hover:text-gray-300">
					חזרה להתחברות
				</a>
			</div>
		{/if}
